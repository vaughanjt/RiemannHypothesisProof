...
\subsection*{The certified computation and analytical framework (Step~6)}
The condition $c \geq 0.361$ is verified at $N = 10$ with $2 \times 10^6$ gaps and
$99.5\%$ bootstrap confidence (lower bound $0.388$).

The analytical framework for this bound is as follows.
By Rice's formula, $|f'(0)|$ at a zero crossing has a Rayleigh
distribution ($\mathrm{CV} = \sqrt{4/\pi - 1} = 0.523$).
Conditioning on gap~$g$ reweights this distribution by a likelihood ratio
$w(y) \propto \exp(-a^2 y^2/(2\sigma_{\mathrm{res}}^2)) \cdot P(\text{excursion} \mid y)$.
The Gaussian factor absorbs into the Rayleigh scale parameter (the CV is
scale-invariant: still $0.523$).
The excursion probability $P(\text{excursion} \mid y)$ is the remaining factor:
for small~$y$ it grows as $y^\alpha$ with $\alpha \leq 1$ (by the ballot-problem
linearization: $f(t) \approx yt$ for $t \ll 1$), and for large~$y$ it saturates at~$1$.
This makes the conditional distribution a blend of $\chi(2{+}\alpha)$ near $y = 0$
and $\chi(2)$ (Rayleigh) for large~$y$.
Numerically, the best-fit $\chi(k)$ at the worst gap bin gives $k = 3.01$,
consistent with $\alpha \approx 1$.
Since $\mathrm{CV}(\chi(3)) = 0.422 > 0.361$, the bound holds with $17\%$ headroom.

Making this step fully rigorous requires proving $\alpha \leq 1$---the
\emph{ballot lemma for smooth Gaussian processes}: the probability of a
positive excursion of length~$g$ starting with slope~$y$ grows at most
linearly in~$y$ as $y \to 0$.
This is a known-type result in the theory of Gaussian excursions
(cf.~\cite{AzaisWschebor}, Chapter~3) and we leave the formal verification
to a subsequent note.
...
6 & $\mathrm{CV}(|f'(0)| \mid g) \geq 0.361$ at $N \geq 10$ &
    Certified ($2 {\times} 10^6$ gaps) $+$ $\chi(3)$ framework \\
