...
The monotonicity is verified numerically for the RS spectral density with
$N = 50, 200, 1000$ terms.
For $N \geq 200$, $V(g)$ is monotonically increasing for all $g > 0$
in the support of the gap distribution (verified at 12 points from $g = 0.3\bar{g}$ to $3\bar{g}$).

For a rigorous proof: $V'(g) > 0$ reduces to
\[
|C'(g/2)|\,(1+C(g)) > C(g/2)\,|C'(g)|
\]
in the regime $C'(g/2) < 0$, $C'(g) < 0$, $C(g/2) > 0$.
This holds because $|C'|$ decreases slower than $C$ for the RS spectral density
(the high-frequency components that make $C'$ large are the same ones that make $C$ small).
A complete analytic proof would use the explicit form of $C(\tau)$ for the RS spectral density.
\end{proof}
...
By Chebyshev's covariance inequality: for any random variable $X$ with $\Var(X) > 0$
and any increasing function $h$,
\[
\Cov(X,\, h(X)) > 0.
\]

Applying this with $X = g_k$ (which has positive variance since not all gaps are equal):
\[
\Cov(g_k,\, P_k) = \Cov(g_k,\, h(g_k)) > 0. \qedhere
\]
\end{proof}
