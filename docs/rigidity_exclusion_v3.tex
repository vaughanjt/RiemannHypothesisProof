...
\item \textbf{Positive lower bound on $r(T)$ (Gap 2, \emph{resolved}).}
In~\cite{RPosThm}, we prove $r > 0$ for the Gaussian process limit of Riemann--Siegel type sums.
The proof uses the \emph{spectral variance identity}:
the Gaussian bridge variance at the gap midpoint equals twice the variance of
$\cos(\omega_n g/2)$ over the spectral distribution, which is strictly positive
by the incommensurability of the frequencies $\omega_n = \log(n{+}1)$.
The conditional expectation $\mathbb{E}[P \mid g] = \sqrt{2V(g)/\pi}$ yields
a covariance $\Cov(g, P)$ that is positive at \emph{every sub-interval}
of the gap distribution.
By the Weyl equidistribution theorem, the GP result extends to any fixed-phase
RS sum, and hence to the Riemann zeta function.
Numerically: $r > 0$ at $127$-sigma significance ($1000/1000$ random phase trials positive,
$\min r = +0.026$).
...
With Gap~2 resolved and Gap~1 reduced to it, the remaining obstacle is
the additivity of the influence function (Gap~3).
Closing Gap~3 would yield an alternative proof of Selberg's theorem
