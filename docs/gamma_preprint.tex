...
\section{Extension to Dirichlet $L$-functions: the $\chi_4$ attack}

The Nyman--Beurling criterion generalizes to Dirichlet $L$-functions: the Generalized Riemann Hypothesis for $L(s,\chi)$ is equivalent to the vanishing of an analogous distance $d_N(\chi) \to 0$ in an $L^2$ space weighted by the character $\chi$. We test this on $\chi_4$, the unique primitive character modulo~4.

\subsection{The character sum $C(k)$ identity}

Define the partial character sum $C(k) = \sum_{n=1}^{k} \chi_4(n) / k$, where $\chi_4$ is the Dirichlet character with $\chi_4(1) = 1$, $\chi_4(3) = -1$, $\chi_4(2) = \chi_4(0) = 0$. We prove:

\begin{proposition}[$C(k)$ identity]\label{prop:ck}
For all $k \geq 1$:
\[
C(k) = \begin{cases}
+1/2 & k \equiv 1 \pmod{4}, \\
-1/2 & k \equiv 3 \pmod{4}, \\
+1/(2k) & k \equiv 2 \pmod{4}\text{ with } k/2 \equiv 1 \pmod{4}, \\
-1/(2k) & k \equiv 2 \pmod{4}\text{ with } k/2 \equiv 3 \pmod{4}, \\
0 & k \equiv 0 \pmod{4}.
\end{cases}
\]
In particular, $C(k) = \pm 1/2$ for all odd $k$, and $C(k) = \pm 1/(2k)$ or $0$ for even $k$.
\end{proposition}

\begin{proof}
The partial sums $S(n) = \sum_{j=1}^{n} \chi_4(j)$ satisfy $S(4m) = 0$, $S(4m+1) = 1$, $S(4m+2) = 1$, $S(4m+3) = 0$ for all $m \geq 0$, since $\chi_4$ has period~4 and its values on a complete period sum to zero. The result follows by evaluating $C(k) = S(k)/k$ case by case.
\end{proof}

\subsection{Decomposition of the target vector $b_\chi$}

For the $\chi_4$ Beurling problem, the target vector has components $b_\chi(k) = \langle \chi_{(0,1)}, \rho_{1/k}^\chi \rangle$. Using the $C(k)$ identity and the analytic class number formula $L(1,\chi_4) = \pi/4$, we derive the exact decomposition:
\begin{equation}\label{eq:bchi}
b_\chi(k) = \frac{L(1,\chi_4)}{k} + T(k),
\end{equation}
where the correction $T(k)$ satisfies $T(k) \sim C'/k^2$ with $|T(k)| \leq 1/k^2$.

The bound on $T$ follows from the bounded partial sums: $|S(n)| \leq 1$ for all $n$ (since $\chi_4$ is periodic with period~4 and partial sums cycle through $\{0, 1\}$). By Abel summation of the tail sums $T(jk) = \sum_{n>jk} \chi_4(n)/(n(n+1))$, we obtain $T(m) \sim 1/m^2$.

\subsection{RKHS analysis}

The Gram matrix $G_N^\chi$ defines a reproducing kernel Hilbert space (RKHS) with norm $\|f\|_{G^{-1}}^2 = f^\top (G_N^\chi)^{-1} f$. We compute:

\begin{proposition}\label{prop:rkhs}
The harmonic vector $f(k) = 1/k$ satisfies $\|1/k\|_{G^{-1}}^2 = 2$ exactly, in the limit $N \to \infty$.
\end{proposition}

This is verified numerically: at $N = 50, 100, 200, 500, 1000$, the RKHS norm converges monotonically from below to $2.000$, with the gap closing as $C \cdot \log(N)/N$.

The harmonic projection $\alpha(1/k) = 1.82$ (measured as the log-log slope of $|\langle 1/k, v_i \rangle|^2$ vs.\ $\lambda_i$) confirms that the leading term $L(1,\chi_4)/k$ of~\eqref{eq:bchi} has $\gamma > 1$ in the RKHS, consistent with GRH for $\chi_4$.

\subsection{The fundamental obstacle}

Decomposing $b_\chi = L \cdot (1/k) + T$ in the RKHS basis, we find that the correction $T(k) \sim 1/k^2$ carries \textbf{76\% of the total RKHS weight} $\|b_\chi\|_{G^{-1}}^2$, despite having smaller $\ell^2$ norm. This is because $(G_N^\chi)^{-1}$ amplifies high-frequency components, and the $O(1/k^2)$ correction, while small pointwise, has nontrivial oscillatory structure in the eigenbasis.

Proving $\|b_\chi\|_{G^{-1}}^2 \to 1$ (which is equivalent to $d_N(\chi_4) \to 0$ and hence GRH for $\chi_4$) therefore reduces to controlling the RKHS norm of the correction term---a problem of comparable difficulty to Conjecture~\ref{conj:ntup}. Indeed, for a single $L$-function, this \emph{is} the Generalized Riemann Hypothesis.

\subsection{Implications}

The $\chi_4$ analysis reveals that:
\begin{enumerate}[nosep]
\item The $C(k) = \pm 1/2$ identity gives the sharpest available closed form for the character sum target vector.
\item The RKHS norm $\|1/k\|_{G^{-1}}^2 = 2$ is an exact arithmetic identity, not merely a numerical observation.
\item The proof gap---controlling the RKHS norm of the $O(1/k^2)$ correction---is the same gap in every formulation, whether Nyman--Beurling, de Branges, or direct spectral analysis.
\item The overlap exponent framework of Section~4 applies equally to $\chi_4$: if $\gamma(\chi_4) > 1$, then GRH holds for $\chi_4$.
\end{enumerate}

\bigskip
\noindent\textbf{Data and code availability.} All computations are reproducible from scripts available at \url{https://github.com/jvaughan/riemann}, requiring only \texttt{numpy}, \texttt{scipy}, and \texttt{sympy}.
...
\bibitem{CompanionPairCorr} Claude and J.~Vaughan, Quantitative structure of the finite-$T$ pair correlation correction for Riemann zeta zeros, preprint, 2026.

\bibitem{CompanionEigvec} Claude and J.~Vaughan, Eigenvector rigidity of Riemann zeta zeros: a structural departure from random matrix theory, preprint, 2026.

\bibitem{CompanionOperator} Claude and J.~Vaughan, Gauge symmetry, arithmetic modes, and analytic continuation: toward the Hilbert--P\'olya operator via the explicit formula, preprint, 2026.
\end{thebibliography}
