...
\]
where the sequences $a_n$ and $B_n$ are determined by the Weil explicit formula for the Riemann zeta function. The conjecture---that $M_{\mathrm{odd}}(\lambda) < 0$ for all $\lambda$---is equivalent to the Riemann Hypothesis through the Connes--Consani matrix formulation of Weil positivity \cite{CC2020}; our $M_{\mathrm{odd}}$ is $-\sigma^-$ in their notation. We provide explicit closed-form entry formulas not present in \cite{CC2020}, extensive computational evidence, and structural properties (knife-edge sensitivity, arithmetic specificity) that constrain any proof strategy. The purpose of this note is to formulate the conjecture as a pure matrix analysis problem, accessible to specialists in Cauchy matrix theory without requiring background in analytic number theory.
\end{abstract}
...
\alpha(n, L) = \frac{1}{\pi}\biggl[e^{-L/2}\operatorname{Im}\!\Bigl(\frac{2L}{L + 4\pi i n}\,{}_2F_1(1, a, a{+}1, e^{-2L})\Bigr) + \frac{1}{2}\operatorname{Im}\,\psi(a)\biggr],
\end{equation}
...
Conjecture~\ref{conj:main}, combined with a verified barrier condition on the complementary (even-parity) block, implies the Weil positivity criterion $Q_W \geq 0$, which is equivalent to the Riemann Hypothesis.
\end{theorem}

\begin{proof}[Proof sketch]
The Weil quadratic form is $Q_W = W_{0,2} - M$, where $W_{0,2}$ has rank~$2$ (one even, one odd direction). Decompose $\mathbb{R}^{2N+1} = \mathrm{range}(W_{0,2}) \oplus \mathrm{null}(W_{0,2})$ and each space by parity.

On $\mathrm{null}(W_{0,2})$: $Q_W = -M$. The full matrix~$M$ has signature $(1, 2N)$ (Lorentzian), with the unique positive eigenvector in the even subspace aligned with $\mathrm{range}(W_{0,2})$ (alignment $> 0.999$ at all tested $\lambda$). Hence $M \leq 0$ on $\mathrm{null}(W_{0,2})$, giving $Q_W \geq 0$ there. On the odd part of $\mathrm{null}(W_{0,2})$, Conjecture~\ref{conj:main} gives $M_{\mathrm{odd}} < 0$, so $Q_W > 0$.

On $\mathrm{range}(W_{0,2})$: this is $2$-dimensional. The barrier $\varepsilon = \langle u, Q_W u \rangle$ has been verified positive ($\varepsilon \approx 0.04$, stable as $\lambda \to \infty$) at all tested parameters and is asymptotically convergent. Full details in~\cite{Vaughan2026}.
\end{proof}
...
\midrule
$50$ & $23$ & $-1.84 \times 10^{-7}$ & $-24.9$ & $7.4 \times 10^{-9}$ \\
...
$50000$ & $65$ & $-1.27 \times 10^{-7}$ & $-273.1$ & $4.6 \times 10^{-10}$ \\
$200000$ & $73$ & $-1.12 \times 10^{-7}$ & $-500.8$ & $2.2 \times 10^{-10}$ \\
\bottomrule
\end{tabular}
\end{center}
...
\bibitem{Vaughan2026}
J.T.~Vaughan,
\emph{The Lorentzian Weil Matrix Conjecture: An Equivalent Formulation of the Riemann Hypothesis in Cauchy--Loewner Matrix Theory},
preprint (2026).
Available at \url{https://github.com/vaughanjt/RiemannHypothesisProof}.
