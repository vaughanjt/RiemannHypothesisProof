...
The locking constant satisfies $\mathcal{Z} \approx 2 + \gamma_{\mathrm{E}} - \log(4\pi)$ to $0.04\%$, where the right side is the known Hadamard constant $\sum_\rho 1/(\rho(1-\rho))$. The $0.04\%$ discrepancy arises from the higher moment $\sum 1/(\gamma^2(\tfrac{1}{4} + \gamma^2))$.
...
This work was conducted over 67 computational sessions using the Connes--Consani--Moscovici framework. All numerical results are reproducible from the Python scripts in the project repository.
