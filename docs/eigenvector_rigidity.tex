...
The full theta function adds only $+0.03$ via its constant phase $-\pi/8 = \arg\Gamma(\tfrac{1}{4})$, the fingerprint of the functional equation.
...
The peak-gap correlation for zeta zeros is 17--20$\times$ larger than for GUE eigenvalues (Table~\ref{tab:peakgap}).
At $T \sim 458$ (499~pairs), Fisher-$z$ gives a 95\% confidence interval $r \in [0.70, 0.79]$.
At $T \sim 2.7\times10^{11}$ (200~pairs), the interval is $r \in [0.74, 0.85]$.
The two intervals overlap, but the point estimates suggest an increasing trend; confirmation at intermediate heights would strengthen this claim.
...
The entire coupling derives from the leading term of~$\theta$ (Table~\ref{tab:phase}).
...
The functional equation's fingerprint is measurable as a $+0.03$ increment from the constant phase $-\pi/8 = \arg\Gamma(\tfrac{1}{4})$.
